package Server.CodeCheckInsert;
 ...
 * 写入临时表（出库）
 */
@WebServlet("/CodeCheckInsertForDBOutBox")
public class CodeCheckInsertForDBOutBox extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public CodeCheckInsertForDBOutBox() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(parameter!=null&&!parameter.equals("")){
			try {
				...
				sta = conn.prepareStatement("exec proc_AssembleOutAllot1_Insert ?,?,?,?,?,?,?");

					sta.setString(1, bean.FOrderID);
					sta.setString(2, bean.FPDAID);
					sta.setString(3, bean.FBarCode);
					sta.setString(4, bean.FStorageInID);
					sta.setString(5, bean.FWaveHouseInID);
					sta.setString(6, bean.FOrgInID);
					sta.setString(7, bean.FHuozhuInID);
