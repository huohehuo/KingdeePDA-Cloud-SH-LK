 ...
 * 条码打印
 */
@WebServlet(urlPatterns = "/GetPGet4BoxUploadData")
public class GetPGet4BoxUploadData extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ResultSet rs = null;
        ArrayList<DownloadReturnBean.DryingGetData> list = new ArrayList<>();
...
//                }
                SearchBean bean = new Gson().fromJson(parameter,SearchBean.class);
...
//                SQL="select convert(float, t6.FQty) as 总数量,convert(float,t6.FVolume) as 总面积,t1.FBillNo as 箱码,t1.FDate as 装箱日期,t3.FName as 制单人,t4.FName as 名称,t5.FName as 单位,(t1.FChang+'x'+t1.FHuang+'x'+t1.FHou) as 规格,t2.FBatchNo as 批号,t2.FLength as 长度,convert(decimal(28,0),t2.FQty-isnull(t2.FQtyOut,0)) as 数量,convert(float,t2.FVolume-isnull(t2.FVolumeOut,0)) as 面积 from t_AssembleRecord t1 inner join t_AssembleRecordentry t2 on t1.FID =t2.FID left join  T_SEC_user  t3 on t1.FBillerID=t3.FUSERID left join t_bd_material_l t4 on  t2.FItemID=t4.fmaterialid left join  T_BD_UNIT_L t5 ON  t2.FUNITID = t5.FUNITID inner join (select sum(FQty-isnull(FQtyOut,0)) as FQty,sum(FVolume-isnull(FVolumeOut,0)) as FVolume,FID from t_AssembleRecordentry where FQty>isnull(FQtyOut,0)  group by FID) t6 on t1.FID =t6.FID  where t1.FBillNo = '"+parameter+"' order by FEntryID";
                SQL="select t2.FNumber as 物料编码,t3.FNumber as 单位编码,t6.FNumber as 仓库编码,t7.FNumber as 库存组织编码,t8.FNumber as 货主编码,t1.FBatchNo as 批号,t1.FLength as 长度,convert(decimal(28,0),t1.FQty) as 数量,convert(float,t1.FVolume) as 面积 from t_AssembleRecord1Temp t1 left join T_BD_MATERIAL t2 on t1.FItemID = t2.FMaterialid  left join T_BD_UNIT t3 on t1.FUnitID=t3.FUnitID   left join t_BD_Stock t6 on t1.FDCStockID=t6.FSTOCKID left join T_ORG_Organizations t7 on t1.FSTOCKORGID = t7.FORGID left join  T_ORG_Organizations t8 on t1.FOWNERID = t8.FORGID   where t1.FPDAID='"+bean.searchType+"' and FOrderID = '"+bean.json+"'";
                Lg.e("GetDryingGetUploadData/SQL"+SQL);
                ...
                if(rs!=null){
                    ...
                    while (rs.next()) {
                        DownloadReturnBean.DryingGetData cBean = downloadReturnBean.new DryingGetData();
...
//                        cBean.FM2Sum					=rs.getString("总面积");
                    ...
                    downloadReturnBean.dryingGetDatas = list;
                    Lg.e("GetDryingGetUploadData"+list.size(),list);
