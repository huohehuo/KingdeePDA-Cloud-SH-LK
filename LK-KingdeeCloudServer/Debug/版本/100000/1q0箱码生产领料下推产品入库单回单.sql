if (exists (select * from sys.objects where name = 'proc_K3ProductIn_Assemble'))
drop proc proc_K3ProductIn_Assemble
go
create proc proc_K3ProductIn_Assemble
(
...
begin tran declare
  ...
  update t_AssembleRecord1  set FInterIDIn=@FInterID, FStatus = 1,FRemark = '简单生产领料单下推产品入库单'  where FBillNo = @FAssemble
