if (exists (select * from sys.objects where name = 'proc_K3SaleStockPushSaleRet'))
drop proc proc_K3SaleStockPushSaleRet
go
create proc proc_K3SaleStockPushSaleRet
(
...
begin tran declare
  ...
  update t_PDABarCodeSign  set FInterIDOut=@FInterID,FDateOutStore=@fdate,FIsOutStore='未出库',FUserOutStore=@FBillerID,FRemark2='销售出库单下推销售退货',FQtyOut=isnull(FQtyOut,0)-t.FQty  from (select a.* from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID) as t where t_PDABarCodeSign.FBarCode=t.FBarCode
 insert into t_PDABarCodeSign_Red(FBarCode,FInterID,FROB,FRemark,FInterID_Before,FBillNO,FStatus,FQtyOut)select a.FBarCode,@FInterID,0,'销售出库单下推销售退货',b.FInterIDOut,@FBillNO,1,-a.FQty from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID 
