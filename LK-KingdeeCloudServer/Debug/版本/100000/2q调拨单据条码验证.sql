if (exists (select * from sys.objects where name = 'proc_AllotBarCode_check2'))
    drop proc proc_AllotBarCode_check2
go
create proc proc_AllotBarCode_check2
(  
...
( 
 ...
 begin
  ...
  begin
	...
	insert into #Tmp11111(FItemID,FUnitID,FQty,FStockID,FStockPlaceID,FBatchNo,FKFPeriod,FKFDate,FBillNo,FExplanation,FRemark3,FRemark4,FRemark7,F_FFF_TEXT1,F_FFF_INTEGER,F_FFF_INTEGER1,F_FFF_INTEGER2,F_FFF_INTEGER3,F_FFF_INTEGER4,F_FFF_Volume,F_TypeID,F_Plies,FRemark5)
	select top 1 FItemID,FUnitID,FQty-isnull(FQtyOut,0)-@FQtyAll,FStockID,FStockPlaceID,FBatchNo,FKFPeriod,FKFDate,FBillNo,@FExplanation,FRemark3,FRemark4,FRemark7,F_FFF_TEXT1,F_FFF_INTEGER,F_FFF_INTEGER1,F_FFF_INTEGER2,F_FFF_INTEGER3,F_FFF_INTEGER4,F_FFF_Volume,F_TypeID,F_Plies,FRemark5 from t_PDABarCodeSign where FBarCode=@FBarCode
...
select FExplanation as 说明,FBillNo,@FNumber as FItemID,FUnitID,convert(float,FQty) as FQty,FStockID,FStockPlaceID,FBatchNo,FKFPeriod,FKFDate,FRemark3 as 实际规格,FRemark4 as 辅助标识,FRemark7 as 生产编号,F_FFF_TEXT1 as 等级,convert(decimal(28,2),ISNull(F_FFF_INTEGER,0)) as 原木长,convert(decimal(28,2),ISNull(F_FFF_INTEGER1,0)) as 原木直径,convert(decimal(28,0),ISNull(F_FFF_INTEGER2,0)) as 板长,convert(decimal(28,0),ISNull(F_FFF_INTEGER3,0)) as 板宽,convert(decimal(28,0),ISNull(F_FFF_INTEGER4,0)) as 板厚,convert(float,ISNull(F_FFF_Volume,0)) as 体积,isnull(F_TypeID,1) as 条码类型,convert(int,F_Plies) as 层数,FRemark5 as 货主编码 from #Tmp11111
drop table #Tmp11111
commit tran 
return;
--------------存在错误
