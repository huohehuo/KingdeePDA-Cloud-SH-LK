if (exists (select * from sys.objects where name = 'proc_AssembleOutAllot1_Insert'))
    drop proc proc_AssembleOutAllot1_Insert
go
create proc proc_AssembleOutAllot1_Insert
(
  @FOrderID varchar(255),--PDA单据号
  @FPDAID varchar(255),  --PDA序列号
  @FBillNo varchar(128), --箱码
  @FDCStockID int, --入库仓库ID
  @FDCSPID int,  --入库仓位ID
  @FSTOCKORGID int,--入库库存组织ID
  @FOWNERID int --入库货主ID
)
as 
