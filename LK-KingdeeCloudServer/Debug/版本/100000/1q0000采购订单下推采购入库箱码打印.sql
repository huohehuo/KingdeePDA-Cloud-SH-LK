if (exists (select * from sys.objects where name = 'proc_PDAAssemblePOOrder_Insert1'))
    drop proc proc_PDAAssemblePOOrder_Insert1
go
create proc proc_PDAAssemblePOOrder_Insert1
(
...
set @FSourceID = dbo.getString(@mainStr,'|',4) --源单单号默认为0
...
set @FBatcnNoIndex = dbo.getString(@mainStr,'|',9) --批号附加值
...
INSERT INTO t_AssembleRecord1(FBillNo,FIndex,FDate,FBillerID,FStatus,FShortDate,FCarNum,FHuZhu,FName,FSourceID,FType) 
values (@FBillNo,@FIndex,@FDate,@FBillerID,0,@FShortDate,@FCarNum,@FHuZhu,'123',@FSourceID,3)
	...
	set @detailIndex=@detailIndex+1 
     ...
     if exists(select 1 from T_BD_UNITCONVERTRATE where   FMASTERID=@FMASTERID and FCURRENTUNITID=@FUnitID)
      select @FVolume=@FQty*FCONVERTNUMERATOR/FCONVERTDENOMINATOR from T_BD_UNITCONVERTRATE where   FMASTERID=@FMASTERID and FCURRENTUNITID=@FUnitID     
       ...
       if exists(select 1 from t_AssembleRecord1 where FID=@FID and FName = '123')
       begin
       select  @FName=  st31.FName from   T_BD_MATERIAL_L st31   where  st31.FMATERIALID = @FMATERIALID
    set  @FName = replace(@FName, reverse(substring(reverse(@FName),1,charindex('*',reverse(@FName)) )) ,'')
    update t_AssembleRecord1 set FName = @FName where FID=@FID
       end
