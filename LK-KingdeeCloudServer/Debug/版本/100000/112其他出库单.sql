if (exists (select * from sys.objects where name = 'proc_K3OtherOutStock'))
drop proc proc_K3OtherOutStock
go
create proc proc_K3OtherOutStock
(
...
begin tran declare
          ...
          select @FInterID=FID from T_STK_MISDELIVERY where FBillNo=@FBillNo 
    set @Fdate = convert(varchar(20),GETDATE(),23)                                       
update t_PDABarCodeSign  set FInterIDOut=@FInterID,FDateOutStore=@fdate,FIsOutStore='已出库',FUserOutStore=@FBillerID,FRemark2='其他出库单',FQtyOut=isnull(FQtyOut,0)+t.FQty  from (select a.* from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID) as t where t_PDABarCodeSign.FBarCode=t.FBarCode
 insert into t_PDABarCodeSign_Red(FBarCode,FInterID,FROB,FRemark,FInterID_Before,FBillNO,FStatus,FQtyOut)select a.FBarCode,@FInterID,0,'其他出库单',b.FInterIDOut,@FBillNO,1,a.FQty from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID 
