...
import com.fangzuo.assist.cloud.Activity.ReViewP14Box2Activity;
import com.fangzuo.assist.cloud.Activity.ReViewP14BoxActivity;
import com.fangzuo.assist.cloud.Adapter.PushDownSubP14PGetListAdapter;
...
public class FragmentPG2Cprk2Detail extends BaseFragment {
    ...
    private List<PGetData> pGetDataList;
...
//    private PushDownSubP1ListAdapter pushDownSubListAdapter;
    private PushDownSubP14PGetListAdapter pushDownSubP14PGetListAdapter;
...
//    private Org mainStoreOrg;//表头带出
    ...
    private int printDataAllTime = 0;
    private int printDataTime = 0;
    private String backBatch = "";
                        ...
                        App.getRService().doIOAction(WebApi.P1PG2CprkUpload, reString, new MySubscribe<CommonResponse>() {
                            ...
                            public void onNext(CommonResponse commonResponse) {
                                ...
                                reBoxBeanDao.deleteInTx(reBoxBeanDao.queryBuilder().where(
                                        ...
                                        ReBoxBeanDao.Properties.FOrderId.eq(mains.get(pos).FOrderId),
                                ...
                                for (int i = 0; i < mains.size(); i++) {
                                    ...
                                    List<PGetData> pGetDatas = pGetDataDao.queryBuilder().where(
                                            PGetDataDao.Properties.FBillNo.eq(mains.get(i).FBillNo),
                                            PGetDataDao.Properties.FAccountID.eq(CommonUtil.getAccountID())
                                    ).build().list();
                                    pGetDataDao.deleteInTx(pGetDatas);
                                }
                            }
...
//                btnBackorder.setClickable(true);
                    ...
                    delete.setNegativeButton("反馈信息", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            DataService.pushBackJson(mContext, FragmentPG2Cprk2Detail.this.getClass().getSimpleName(), Hawk.get(Config.Company,""));
                        }
                    });
                    delete.create().show();
                }
...
//                }
                break;
            case EventBusInfoCode.PrintBox_OK:
                String pok = (String) event.postEvent;
                //遍历打印数据，
                if (printDataTime<=printDataAllTime-1){
                    doPrintData(printDataTime);
                }else{
                    Addorder();
                }
                break;
            case EventBusInfoCode.PrintBox_Error:
                String poerror = (String) event.postEvent;
                Toast.showText(mContext,poerror);
                //删除已经添加的箱码
                reBoxBeanDao.deleteInTx(reBoxBeanDao.queryBuilder().where(
                        ReBoxBeanDao.Properties.FOrderId.eq(ordercode),
                        ReBoxBeanDao.Properties.FAccountID.eq(CommonUtil.getAccountID())
                ).build().list());
                LoadingUtil.dismiss();
                break;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_pg2cprk2_detail, container, false);
...
//        ordercode = CommonUtil.createOrderCode(activityPager.getActivity()+fidcontainer.get(0));//单据编号
        ...
        if (pushDownMains.size() > 0) {
            ...
            List<PGetData> list1 = pGetDataDao.queryBuilder().where(
                    PGetDataDao.Properties.FID.eq(pushDownMain.FID),
                    PGetDataDao.Properties.FAccountID.eq(CommonUtil.getAccountID())
            ).build().list();
            pGetDataList.addAll(list1);
            Lg.e("得到列表数据",pGetDataList);
            if (pGetDataList.size() > 0) {
                pushDownSubP14PGetListAdapter = new PushDownSubP14PGetListAdapter(mContext, pGetDataList);
                lvPushsub.setAdapter(pushDownSubP14PGetListAdapter);
                pushDownSubP14PGetListAdapter.notifyDataSetChanged();
            } else {
...
//        Addorder();
        ...
        //获取列表数量，作为打印数量
        printDataAllTime = pushDownSubP14PGetListAdapter.getCount();

        printDataTime=0;
        backBatch="";
        new AlertDialog.Builder(mContext)
                .setTitle("本次将要装"+pushDownSubP14PGetListAdapter.getCount()+"箱，将需要"+(MathUtil.toD(activityPager.getPrintNum())*pushDownSubP14PGetListAdapter.getCount()*2)+"个标签，是否继续打印")
                .setPositiveButton("确认", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        LoadingUtil.showDialog(mContext, "正在打印条码数据...");
                        doPrintData(0);
                    }
                })
    ...
    PGetData getData;
    private void doPrintData(int num){
        try {
            getData = pGetDataList.get(num);
        }catch (Exception e){
            return;
        }
        //??箱码生成太快？导致打印一半，被下一个箱码打印任务给终止掉。。。
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        Lg.e("执行打印操作。。。"+printDataTime);
        ...
        String pdata = Hawk.get(Info.user_id, "") + "|" + activityPager.getCarNo() + "|" + activityPager.getOrgIn().FNote
                ...
                + "|" + activityPager.getOrgOut().FOrgID + "|" + activityPager.getOrgIn().FOrgID+"|"+getData.FWide+"|"+backBatch;
        App.getRService().doIOAction(WebApi.PrintData4Package2, pdata, new MySubscribe<CommonResponse>() {
            @Override
            public void onNext(CommonResponse commonResponse) {
                super.onNext(commonResponse);
                if (!commonResponse.state) return;
                ...
                if (null != dBean && dBean.printDataBeans.size() > 0) {
                    ...
                    backBatch = printDataBeans.get(0).FBatch;
                    ReBoxBean boxBean = new ReBoxBean();
                    boxBean.FPCS = getData.PCS;
                    boxBean.FM3 = getData.M3;
                    boxBean.FWide = getData.FWide;
                    boxBean.FBoxCode = printDataBeans.get(0).FBoxCode;boxBean.FID = pushDownMain.FID;boxBean.activity = activity;
                    boxBean.FBatch = printDataBeans.get(0).FBatch;boxBean.FAccountID = CommonUtil.getAccountID();boxBean.FOrderId = ordercode;
                    reBoxBeanDao.insert(boxBean);
...
//                    daoSession.getPrintHistoryDao().insert(printHistory);
                    printDataTime++;
                    try {
                        CommonUtil.doPrint4P1BoxCode2(zpSDK,mContext, dBean.printDataBeans,activityPager.getPrintNum());
                    } catch (Exception e) {
                        EventBusUtil.sendEvent(new ClassEvent(EventBusInfoCode.PrintBox_OK,"获取箱码成功"));
                    }
//                    Addorder();
                } else {
                    EventBusUtil.sendEvent(new ClassEvent(EventBusInfoCode.PrintBox_Error,"打印失败-生成条码失败，请重试"));
                }
            }
            ...
            public void onError(Throwable e) {
                super.onError(e);
                LoadingUtil.dismiss();
//                Toast.showText(mContext, "生成条码失败，请重试"+e.getMessage());
                EventBusUtil.sendEvent(new ClassEvent(EventBusInfoCode.PrintBox_Error,"打印失败"+e.getMessage()));
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    for (int i = 0; i < container.size(); i++) {
                        PushDownSub pushDownSub = container.get(i);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_checkorder:
                ...
                startNewActivity(activityPager, ReViewP14Box2Activity.class,
