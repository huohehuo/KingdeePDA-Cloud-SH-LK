...
public class ReViewDataAdapter extends BaseAdapter implements View.OnClickListener {
    Context context;
    ...
    public ReViewDataAdapter(Context context, List<T_Detail> detail, List<Boolean> isCheck) {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.item_review_data_list, null);
...
//                viewHolder.productxh.setText("物料名:" + detail.get(i).FProductName);
        ...
        viewHolder.tvBarCode.setText("生产日期:" + detail.get(i).FProduceDate);
...
//                ||detail.get(i).activity==Config.DgInActivity||detail.get(i).activity==Config.SimpleInActivity){
            viewHolder.tvStorenum.setText("数量:" + detail.get(i).FRealQty +detail.get(i).FUnit);
