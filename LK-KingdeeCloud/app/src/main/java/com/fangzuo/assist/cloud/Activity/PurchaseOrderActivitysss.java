...
//        ordercode = createOrderCode(share);
//        loadBasicData();
...
//    }
//    //生成单据编号
//    public long createOrderCode(ShareUtil share){
//        Long ordercode=0l;
...
//        } else {
//            //当不是当天时，生成新的单据，重新计算
//            if (String.valueOf(share.getPOOrderCode()).contains(getTime(false))){
...
//                    unitName = unit.FName;
//                    unitrate = MathUtil.toD(unit.FMeasureUnitID);
...
//                startNewActivity(ReViewActivity.class, R.anim.activity_fade_in, R.anim.activity_fade_out, false, b1);
//                break;
//            case R.id.tv_date:
//                getdate();
//                break;
...
//                        products = productDao.queryBuilder().where(ProductDao.Properties.FAuxUnitID.eq(barCodes.get(0).FItemID)).build().list();
...
//            product = list.get(0);
////            spUnit.setAuto(mContext,product.FAuxUnitID,default_unitID,SpinnerUnit.Id);
//            tvorisAuto(product);
//        } else {
...
//        edPricesingle.setText(df.format(MathUtil.toD(product.FAuxUnitID)));
...
////                        T_DetailDao.Properties.FProductId.eq(product.FMaterialid),
...
////            t_detail.FProductId = product.FStockID;
