...
public class CountOffAdapter extends RecyclerArrayAdapter<T_Detail> {
    public CountOffAdapter(Context context) {
        super(context);
    }
...
//    }
    ...
    public String getAllPcs(){
        int num=0;
        for (int i = 0; i < getAllData().size(); i++) {
            num=num+MathUtil.toInt(getAllData().get(i).FCfQty);
        }
        return num+"";
    }
    public String getAllM2(){
        Double num=0.0;
        for (int i = 0; i < getAllData().size(); i++) {
            num=MathUtil.sum(num+"",MathUtil.toD(getAllData().get(i).FCfM2)+"");
        }
        return num+"";
    }
    @Override
...
//        }
    ...
    class MarkHolder extends BaseViewHolder<T_Detail> {
        ...
        public MarkHolder(ViewGroup parent) {
            super(parent, R.layout.item_wort_list);
        ...
        @Override
        public void setData(T_Detail data) {
            super.setData(data);
            tv_Lenght.setText(data.FCfLenght+"");
            tv_Pcs.setText(data.FCfQty);
            tv_M2.setText(data.FCfM2);
        }
    }
