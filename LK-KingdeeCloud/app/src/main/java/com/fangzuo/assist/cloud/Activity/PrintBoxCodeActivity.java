...
import com.fangzuo.assist.cloud.databinding.ActivityPrintBoxCodeBinding;
...
public class PrintBoxCodeActivity extends BaseActivity {

    ActivityPrintBoxCodeBinding binding;
//    zpBluetoothPrinter zpSDK;
    private PrintBoxCodeRyAdapter adapter;
//    BlueToothBean bean;
...
//                Toast.showText(mContext, "扫描结果：" + res.getResult().getText());
    ...
    protected void initView() {
        binding = DataBindingUtil.setContentView(this, R.layout.activity_print_box_code);
//        zpSDK=new zpBluetoothPrinter(this);
        //摄像头初始化
        mCaptureManager = new ScanManager(PrintBoxCodeActivity.this, binding.zxingBarcodeScanner);
        mCaptureManager.initializeFromIntent(getIntent(), savedInstanceState);
        binding.toolbar.tvTitle.setText("箱码补打");
        binding.ryPrintHistory.setAdapter(adapter = new PrintBoxCodeRyAdapter(this));
...
//                    mCaptureManager.onPause();
    ...
    private void getPrintHistory(String type,String string){
        ...
        App.getRService().doIOAction(WebApi.PrintOutCheck4BoxCode, gson.toJson(searchBean), new MySubscribe<CommonResponse>() {
            @Override
...
//                Lg.e("获得打印数据：",commonResponse.returnJson);
                ...
                if (dBean.wortPrintDatas.size()>0){
                    adapter.clear();
                    adapter.addAll(dBean.wortPrintDatas);
//                    showMsg(dBean.printHistories.get(0));
    ...
    //展示打印数据
    private void showMsg(final WortPrintData data){
...
//        }).start();
    ...
    public static void start(Context context){
        Intent intent = new Intent(context,PrintBoxCodeActivity.class);
