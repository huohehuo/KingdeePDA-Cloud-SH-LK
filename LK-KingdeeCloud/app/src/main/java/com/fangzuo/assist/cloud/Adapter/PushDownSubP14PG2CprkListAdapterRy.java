...
public class PushDownSubP14PG2CprkListAdapterRy extends RecyclerArrayAdapter<PushDownSub> {
    public PushDownSubP14PG2CprkListAdapterRy(Context context) {
        super(context);
    }
...
//    }
    ...
    public String getAllPcs(){
        Double num=0d;
        for (int i = 0; i < getAllData().size(); i++) {
            num=num+MathUtil.toD(getAllData().get(i).FPCS);
        }
        return num+"";
    }
    public String getPcsing(){
        Double num=0d;
        for (int i = 0; i < getAllData().size(); i++) {
            num=num+MathUtil.toD(getAllData().get(i).FQtying);
        }
        return num+"";
    }

    public String getAllM3(){
        Double num=0.0;
        for (int i = 0; i < getAllData().size(); i++) {
            num=MathUtil.sum(num+"",getAllData().get(i).FM3);
        }
        return num+"";
    }
    public String getAllPcsSplit(){
        int num=0;
        for (int i = 0; i < getAllData().size(); i++) {
            num=num+MathUtil.toInt(getAllData().get(i).FM3);
        }
        return num+"";
    }
    @Override
...
//        }
    ...
    class MarkHolder extends BaseViewHolder<PushDownSub> {
        ...
        public MarkHolder(ViewGroup parent) {
            super(parent, R.layout.item_pg2cgrk_p1_list);
            tv_model1 =   $(R.id.tv_model1);
        ...
        @Override
        public void setData(PushDownSub data) {
            super.setData(data);
            tv_model1.setText(data.FPCS);
            tv_model2.setText(data.FQtying);
            tv_model3.setText(data.FM3);
            tv_model4.setText(data.FLevel);
