...
import com.fangzuo.assist.cloud.Adapter.SuppliersRyAdapter;
...
public class SpinnerSupplierDlg extends RelativeLayout {
    ...
    private SuppliersRyAdapter adapter;
    ...
    public SpinnerSupplierDlg(final Context context, AttributeSet attributeSet) {
...
//                    break;
        ...
        adapter = new SuppliersRyAdapter(context);
    ...
    private void showDlg(Context context){
        ...
        App.getRService().doIOAction(WebApi.SUPPLIERSEARCHLIKE, new Gson().toJson(new SearchBean(SearchBean.product_for_like,new Gson().toJson(s2Product))), new MySubscribe<CommonResponse>() {
            ...
            public void onNext(CommonResponse commonResponse) {
                ...
                if (dBean.suppliers!=null && dBean.suppliers.size()>0){
                    Lg.e("获得供应商数据",dBean.suppliers);
                    adapter.clear();
                    adapter.addAll(dBean.suppliers);
                    adapter.notifyDataSetChanged();
                }else{
                    adapter.clear();
                    adapter.notifyDataSetChanged();
        ...
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            ...
            public void onItemClick(int position) {
                Suppliers storage = adapter.getAllData().get(position);
                Lg.e("点击供应商：",storage);
...
//    }
    ...
    public SuppliersRyAdapter getAdapter() {
        return adapter;
    }
