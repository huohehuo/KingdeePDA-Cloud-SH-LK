...
import com.fangzuo.assist.cloud.Dao.CountOffBeanList;
...
public class CountOffListAdapter extends RecyclerArrayAdapter<CountOffBeanList> {
    StringBuffer stringBuffer;
    public CountOffListAdapter(Context context) {
        super(context);
    }
...
//        }
    ...
    class MarkHolder extends BaseViewHolder<CountOffBeanList> {

        private TextView tv_Number;
        private TextView tv_Lenght;

        public MarkHolder(ViewGroup parent) {
            super(parent, R.layout.item_countoff_list);
            tv_Number =   $(R.id.tv_number);
            tv_Lenght =   $(R.id.tv_lenght);

        }

        @Override
        public void setData(CountOffBeanList data) {
            super.setData(data);
            tv_Number.setText(data.FCountNumber+"");
            stringBuffer = new StringBuffer();
            for (T_Detail bean:data.list) {
                stringBuffer.append(bean.FCfLenght).append(" / ");
            }
            Lg.e("报错对象",stringBuffer.toString());
            if (stringBuffer.toString().length()>3){
                tv_Lenght.setText(stringBuffer.toString().substring(0,stringBuffer.toString().length()-3));
            }else{
                tv_Lenght.setText(stringBuffer.toString());
            }
