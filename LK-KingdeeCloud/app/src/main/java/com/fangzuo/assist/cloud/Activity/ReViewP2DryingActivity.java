...
import com.fangzuo.assist.cloud.databinding.ActivityReViewP2DryingBinding;
...
public class ReViewP2DryingActivity extends BaseActivity {
    ActivityReViewP2DryingBinding binding;

    private int activity;
    private String  boxCode;
    private List<DryingGetData> list;
    private List<T_main> mainsList;
    private ReViewP24DryingAdapter adapter;
...
//                    e.printStackTrace();
    ...
    protected void initView() {
        binding = DataBindingUtil.setContentView(this, R.layout.activity_re_view_p2_drying);
...
//        List<WortPrintData> listTemp = DataModel.getP2SplitDetail(mContext,boxCode);
        ...
        Lg.e("列表数据：",list);
        adapter = new ReViewP24DryingAdapter(mContext, list, isCheck);
...
//                                adapter.notifyDataSetChanged();
                        ...
                        pBean.list = data;
                        Gson gson = new Gson();
                        App.getRService().doIOAction(WebApi.CodeOnlyDelete4Drying, gson.toJson(pBean), new MySubscribe<CommonResponse>() {
                            @Override
                            public void onNext(CommonResponse commonResponse) {
                                super.onNext(commonResponse);
                                if (!commonResponse.state) return;
